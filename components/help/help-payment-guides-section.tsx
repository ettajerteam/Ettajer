...
import { PAYMENT_PLATFORM_GUIDES } from "@/lib/help/help-checklist";
...
export function HelpPaymentGuidesSection() {
  const { copy } = useHelpLocale();
  const g = copy.paymentGuides;
      ...
      <div className="mx-auto max-w-6xl px-4 py-12 md:px-6 md:py-16">
        ...
        <div className="md:hidden">
          <LandingCarousel
            ...
            gap={12}
          >
            {PAYMENT_PLATFORM_GUIDES.map((item, index) => {
              ...
              return (
                <HelpMobileCard
                  key={item.id}
                  className="flex min-h-[13.5rem] flex-col"
                >
                  <span
                    className="flex h-12 w-12 items-center justify-center rounded-2xl text-[11px] font-bold tracking-tight text-white"
        ...
        <ul className="mt-8 hidden gap-3 md:grid md:grid-cols-2 lg:grid-cols-4">
          {PAYMENT_PLATFORM_GUIDES.map((item, index) => {
            const itemCopy = g.items[index];
            if (!itemCopy) return null;
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3">
                  <span
                    className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl text-[10px] font-bold tracking-tight text-white"
                    style={{
                      backgroundColor: item.accent,
                      color: item.id === "snapchat" ? "#111" : undefined,
                    }}
                  >
                    {item.badge}
                  </span>

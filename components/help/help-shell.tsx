...
const FOOTER_GROUP_INDICES = [2, 5, 0] as const;
...
export function HelpShell({ children, breadcrumb }: HelpShellProps) {
  const { locale, copy, isRtl } = useHelpLocale();
  const shell = copy.shell;
  const footerGroups = getFooterNavGroups(locale);
  const footerNav = FOOTER_GROUP_INDICES.map((index) => footerGroups[index]).filter(Boolean);

  const mobileLinks = [
    { href: "/help", label: shell.getHelp },
    { href: "/contact", label: shell.contactSupport },
        ...
        <div className="mx-auto flex h-[3.25rem] max-w-6xl items-center justify-between gap-3 px-4 md:h-auto md:gap-6 md:px-6 md:py-3.5">
          ...
          <div className="hidden items-center gap-3 md:flex">
            <div className="hidden min-w-[8.5rem] sm:block">
              <LandingLanguageSwitcher label={shell.language} />
            </div>
            <Link
              href="/signup"
          ...
          <div className="flex items-center gap-2 md:hidden">
            <LandingLanguageSwitcher variant="select" className="sm:hidden" />
            <Link
              href="/contact"
...
function HelpMobileMenu({
  ...
  onClose: () => void;
  shell: ReturnType<typeof useHelpLocale>["copy"]["shell"];

...
import { buildPinterestCatalogFeedTsv } from "@/lib/pinterest-product-feed";
...
export async function GET(request: Request, context: RouteContext) {
  try {
    ...
    const requiredKey = pinterest.catalogFeedToken?.trim() || null;
    if (requiredKey) {
      const provided = new URL(request.url).searchParams.get("key")?.trim() || "";
      if (provided !== requiredKey) {
        return NextResponse.json(
          { message: "Invalid or missing feed key" },
          { status: 401 }
        );
      }
    }

    const tsv = buildPinterestCatalogFeedTsv({
    ...
    return new NextResponse(tsv, {
      ...
      headers: {
        ...
        "Content-Disposition": `inline; filename="${store.slug}-pinterest-catalog.tsv"`,
        "X-Content-Type-Options": "nosniff",
      },
    });
  } catch (error) {
    console.error("[pinterest-catalog-feed]", error);
    return NextResponse.json(
      { message: "Failed to build catalog feed" },
      { status: 500 }
    );
  }
}
